...
class Excel extends \yii\base\Widget
{
	 ...
	 * $hap - cтрока шапки таблицы, уст. в параметрах виджета 
	 */
	public function executeColumns(&$activeSheet = null, $models, $columns = [], $headers = [], $hap)
	{
		
		$kubik = rand(1,3);
		if($kubik == 1)
			$activeSheet->getStyle('A1')->getFont()->getColor()->applyFromArray(['rgb' => '008080']);  // Раскраска символов в ячейке
		if($kubik == 2)
			$activeSheet->getStyle('A1')->getFont()->getColor()->applyFromArray(['rgb' => '00008B']);  // Раскраска символов в ячейке
		if($kubik == 3)
			$activeSheet->getStyle('A1')->getFont()->getColor()->applyFromArray(['rgb' => '8B0000']);  // Раскраска символов в ячейке
		// Подписываем лист
		//$activeSheet->setTitle(substr($hap,0,20));
		$activeSheet->getStyle('A1')->getFont()->setBold(true);  // Уст. жирного шрифта
		$activeSheet->setCellValue('A1', $hap);   // Строка шапки таблицы

//		$activeSheet->setCellFont('A1','Bold = True');
...
//						return;
						...
						if (isset($column['header'])) {
							...
							$header = $model->getAttributeLabels($column['attribute']);
...
//					return;
				...
				if($column['attribute']=='date') {

					$activeSheet->setCellValue($col . $row, date("d.m.Y", strtotime($column_value)));
				}
				else
	...
	public function executeColumns_loc(&$activeSheet = null, $model, $columns = [], $headers = [], $hap)
	{
		...
		$row = 3;
		$char = 26;
		$head_attr = $model->attributeLabels();
				...
				foreach ($columns as $key=>$column) {
					...
					if (is_array($column)) {
						...
						if (isset($column['header'])) {
							...
							$header = $head_attr[$column['attribute']];
...
//        fputs($f,__DIR__ . '/LogReport/');
	...
	public function run()
	{
		if ($this->mode == 'export') 
		{
			...
			foreach (range('A-H', $sheet->getActiveSheet()->getHighestDataColumn()) as $col) {
	    	...
	    	if ($this->isMultipleSheet) {
	    		...
	    		foreach ($this->models as $title => $models) {
	    			...
	    			$this->executeColumns($worksheet[$index], $models, $this->populateColumns($columns), $headers, $this->hap);
	    			$index++;
	    		}
	    	} else {
...
//					->setAutoSize(true);
				...
				if($this->data_model)
	    			$this->executeColumns($worksheet, $this->models, isset($this->columns) ? $this->populateColumns($this->columns) : [], isset($this->headers) ? $this->headers : [], $this->hap);
				else
					// Из модели без использования таблиц SQL сервера
					$this->executeColumns_loc($worksheet, $this->models, isset($this->columns) ? $this->populateColumns($this->columns) : [], isset($this->headers) ? $this->headers : [], $this->hap);
	    	}
